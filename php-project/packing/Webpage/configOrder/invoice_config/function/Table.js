$(document).ready(function (){
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      //click edit
      var id_edit_invoice = null;
      $('#invoice-no').on('click', '.edit-btn', function () {
        id_edit_invoice = $(this).data('id');
        var text_id = "WHERE invoice_id=" + "'" + id_edit_invoice + "'"
        ...
        $.ajax({
          ...
          dataType: "json",
          contentType: "application/json; charset=utf-8",
          data: JSON.stringify(value_invoice),

          success: function (response) {
            var rep = response['Message']
            $('#edit-option-flag-invoice').val(rep[0].flag);
            $('#edit-invoice-name').val(rep[0].invoice_name);
          }
        });
      });
      //button uptade
      $('#update-invoice').on('click', function () {
        var textinvoice_id = "invoice_id=" + "'" + id_edit_invoice + "'"
        var option_flag = $('select[name=edit-option-flag-invoice]').val();
        var edit_invoice_name = $('input[name=edit-invoice-name]').val();
        var text_value = "flag=" + "'" + option_flag + "'" + "," + "invoice_name=" + "'" + edit_invoice_name + "'"

        var data_edit_invoice = {
          "table": "config_invoice_no",
          "operation": "update",
          "sections": "",
          "condition": "",
          "sorted": textinvoice_id,
          "header_fm": "",
          "value_fm": text_value
        }

        if (edit_invoice_name != "") {
          $.ajax({
            type: "POST",
            url: "http://10.19.5.106:5001/api/fConfCommand",
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data_edit_invoice),
      ...
      //delete invoice no
      var id_del_invoice = null;
      $('#invoice-no').on('click', '.delete-btn', function () {

        $('#delete_confirm_invoice').modal('show');
        id_del_invoice = $(this).data('id');



        var where_invoice = "WHERE invoice_id=" + "'" + id_invoice + "'"

        var select_invoice = {
          "table": "config_invoice_no",
          "operation": "select",
          "sections": "*",
          "condition": where_invoice,
          "sorted": "",
          "header_fm": "",
          "value_fm": ""
        }
        ...
        $.ajax({
          ...
          dataType: "json",
          contentType: "application/json; charset=utf-8",
          data: JSON.stringify(select_invoice),

          success: function (response) {
            var rep = response['Message']
            var ID = rep[0].invoice_id
            var Name = rep[0].invoice_name
      ...
      $('#Delete_invoice').on('click', function () {
        var text_id = "invoice_id=" + "'" + id_del_invoice + "'"
        data_delete = {
          "table": "config_invoice_no",
          "operation": "delete",
          "sections": "",
